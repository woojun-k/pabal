...
class EditMessageCommandHandlerTest {
    ...
    @InjectMocks
    private EditMessageCommandHandler handler;

    @Test
    void handle_rechecks_sendable_active_membership_before_editing_message() {
        UUID tenantId = UUID.randomUUID();
        UUID roomId = UUID.randomUUID();
        UUID messageId = UUID.randomUUID();
        UUID requesterId = UUID.randomUUID();
        PersistedMessage message = persistedMessage(tenantId, roomId, messageId, requesterId);
        EditMessageCommand command = new EditMessageCommand(tenantId, roomId, messageId, requesterId, "updated");
    ...
    @Test
    void handle_rejects_edit_when_sender_is_no_longer_active_member() {
        UUID tenantId = UUID.randomUUID();
        UUID roomId = UUID.randomUUID();
        UUID messageId = UUID.randomUUID();
        UUID requesterId = UUID.randomUUID();
        ...
        assertThatThrownBy(() -> handler.handle(new EditMessageCommand(tenantId, roomId, messageId, requesterId, "updated")))
    ...
    @Test
    void handle_rejects_edit_when_room_is_not_sendable() {
        UUID tenantId = UUID.randomUUID();
        UUID roomId = UUID.randomUUID();
        UUID messageId = UUID.randomUUID();
        UUID requesterId = UUID.randomUUID();
