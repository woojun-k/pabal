...
--  - 애플리케이션 레벨 검증은 UX/빠른 실패용이고, 동시성 race condition의 최종 방어는 DB 제약이다.
-- =====================================================================
...
CREATE TABLE IF NOT EXISTS chat_room_member (
    ...
    FOREIGN KEY (chat_room_id) REFERENCES chat_room(id) ON DELETE RESTRICT,
...
CREATE INDEX IF NOT EXISTS idx_chat_room_member_room_left_at
    ON chat_room_member (chat_room_id, left_at);
...
CREATE TABLE IF NOT EXISTS message (
    ...
    CONSTRAINT chk_message_deleted_has_deleted_at
...
CREATE INDEX IF NOT EXISTS idx_message_room_created
    ON message (chat_room_id, created_at DESC, id DESC);
