...
- `pabal-tenant-infrastructure`의 persistence + scheduler 계층은 domain/contract를 따라잡았다: `V7__tenant_registration_tables.sql`이 `verification_expires_at`(NOT NULL)/`activation_expires_at`(nullable), 5-status CHECK, status별 timestamp CHECK를 테이블 단위로 정의하며, `TenantRegistrationEntity`는 새 `TenantRegistrationState` 13-arg 생성자 순서에 맞춰 두 timestamp를 매핑한다. `TenantRegistrationRepositoryImpl.OPEN_STATUSES`는 `DOMAIN_VERIFIED`/`REVERIFICATION_REQUIRED`를 포함한 4개 open status를 사용하고, `TenantRegistrationExpirationScheduler.reverifyLapsedRegistrations()`가 lapsed `DOMAIN_VERIFIED` registration을 도메인 메서드 `requireReverification(now)`를 통해서만 `REVERIFICATION_REQUIRED`로 전이하는 sweep을 `pabal.tenant.registration.reverification-sweep-delay-ms`(기본 600000ms) 간격으로 실행한다.
...
- [x] `V7__tenant_registration_tables.sql`에 `verification_expires_at`/`activation_expires_at` 컬럼과 5-status CHECK constraint 반영
