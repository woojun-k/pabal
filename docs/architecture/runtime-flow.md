...
flowchart LR
    ...
    client --> send["SEND /app/chat.message.send|edit|delete|chat.typing.start|stop"]
    send --> controller["ChatRealtimeCommandController"]
    controller --> command["Message command / SendTypingCommand"]
    command --> handler["Message command handler / SendTypingCommandHandler"]
    handler --> access["ChatRoomAccessSupport"]
