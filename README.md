...
- 메시지 idempotency: `tenantId + chatRoomId + senderId + clientMessageId` 기준 선조회와 DB unique constraint 번역으로 중복 전송 결과 반환
- 메시지 수정/삭제: 작성자 본인만 가능, 삭제 메시지는 재수정/재삭제 방지
- 읽음 처리: 메시지 sequence 기반 read cursor 전진, cursor가 실제로 전진할 때만 read event 발행
- 조회 API: 내 방 목록, 메시지 cursor page, 단건 메시지, unread count, 방 목록 unread batch 집계
...
- 개발 지원: local/test profile용 JWT 발급 endpoint, `.env.local`/`.env.test`, local Docker Compose, Flyway + PostgreSQL/Redis Testcontainers
...
- PostgreSQL / Redis Testcontainers, JUnit 5, Mockito
- Lombok
...
`local` profile은 Spring Boot Docker Compose 연동으로 `compose.local.yaml`의 PostgreSQL 18.3, Redis 8.6 컨테이너를 사용합니다. Docker가 실행 중이어야 합니다.
...
- 테스트는 H2를 사용하지 않고 Flyway migration을 그대로 적용한 PostgreSQL / Redis Testcontainers 기준으로 검증
...
테스트 프로필은 Flyway 스키마를 기준으로 PostgreSQL Testcontainers를 사용하고, Redis가 필요한 케이스는 Redis Testcontainers를 함께 사용합니다.
